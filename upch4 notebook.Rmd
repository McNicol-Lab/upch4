...
```{r}
daily <- lapply(paste(loc, "fluxnet-ch4-data/daily/", files, sep = ""), read_csv)
names(daily) <- site.names
# str(daily)
```
...
```{r}
names(daily[[1]])
head(daily[[1]]$TIMESTAMP)
```

Write and apply function to expand `TIMESTAMP` into`Year`, `Month`, `Week`, `Day`, and `DOY` variables to facilitate merging with other data:

```{r}
fix_weeks <- function(daily_data) {
  ...
  dplyr::select(Year, Month, Week, Day, DOY, everything(), - Date, -TIMESTAMP)
}

daily <- lapply(daily, fix_weeks)
# daily[[2]] # check it worked
```

Check that FCH4 daily mean is -9999 when missing (not already gap-filled). Try `daily[[2]]` (BWGum), which has missing data at the start of the record:
