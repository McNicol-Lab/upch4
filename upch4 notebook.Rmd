...
```{r}
sites <- read_csv("/Volumes/LACIE SHARE/Stanford CH4/upch4_local/fluxnet-ch4-data/metadata/fluxnet-ch4-site-metadata.csv") %>% 
  mutate(ID = paste(substr(ID, 1, 2), substr(ID, 4, 6), sep = "")) 
...
```{r}
loc <- "/Volumes/LACIE SHARE/Stanford CH4/upch4_local/grids/global-canopy-height/"

setwd(loc)
raster.names <-list.files(loc, pattern = "tif$", full.names = FALSE)
stack <- stack(raster.names)
...
```{r}
stack <- stack("/Volumes/LACIE SHARE/Stanford CH4/upch4_local/grids/computed/Rpot_msc.nc")
