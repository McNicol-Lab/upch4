...
Notes:
  ...
  + Total FCH4 uncertainty `FCH4_F_UNC` is random `FCH4_F_RANDUNC` and gap-filling `FCH4_F_ANNOPTLM_UNC` uncertainty summed in quadrature.
...
daily_subset <- daily_flat %>% 
  ...
  dplyr::select(ID, Year, Month, Week, Day, DOY,    # descriptive data
         ...
         LE = LE_F_ANN, H = H_F,  # ecosystem energy fluxes
         TA = TA_F, PA = PA_F, RH = RH_F, VPD = VPD_F, P = P_F, USTAR = USTAR, WS = WS_mean,    # meteorology 
