...
```{r}
writeRaster(Rpot_msc, paste(loc, "grids/computed/Rpot_msc.nc", sep = ""), format="CDF")
```
...
modis_dsp_nas %>% 
  ...
  write.csv(paste(loc, "/modis/modis-qc/despiking_na_effects.csv")
```
...
```{r}
write.csv(modis_frozen, paste(loc, "/modis/modis-processed/modis-processed.csv",
          row.names = FALSE)
```
...
```{r}
earthenv[earthenv$ID == "USDPW", is.na(earthenv[earthenv$ID == "USDPW", ]) ] <- earthenv[earthenv$ID == "USLA1", is.na(earthenv[earthenv$ID == "USDPW", ]) ] 
earthenv[earthenv$ID == "USWPT", is.na(earthenv[earthenv$ID == "USWPT", ]) ] <- earthenv[earthenv$ID == "USOWC", is.na(earthenv[earthenv$ID == "USWPT", ]) ] 
```
