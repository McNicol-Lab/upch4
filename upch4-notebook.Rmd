...
di_curve %>%
  ...
  labs(y = expression(atop("Extraplation Variance", "(nmol m"^{-2}*" s"^{-1}*")"))) +
  my_theme
ggsave(paste0(loc, "dissimilarity/di_vs_variance.png", sep = ""),
...
##### Visualize Bootstrap Timeseries
...
fillvalue <- ncatt_get(tgch4, "sd_ch4", "_FillValue")
...
r <- list()
for(i in 1:216) {
  r[[i]] <- raster(t(tgch4_sd[, , i]), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat), crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0"))
}
...
sd_sums <- cellStats(r_stack[[1:12]], sum)
