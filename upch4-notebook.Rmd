...
```{r}
fluxes <- read_csv(paste(loc, "fluxnet-ch4-data/daily_flat/daily_subset_meta.csv", sep = ""))
modis_geo <- read_csv(paste(loc, "training-data/modis_geospatial.csv", sep = ""))
names(fluxes)
names(modis_geo)
```

```{r}
flux_modis_geo_daily <- fluxes %>% left_join(modis_geo)
...
Remove redundant variable names:
  ...
  - -Site.Name, -Data.Policy, -Site.Classification, -Upland.Class, -DOI.Reference, -Site.Personnel, -Base.File.Host
  
Also remove `Upland Class` which is not applicable to the wetland dataset.

```{r}
all_data <- read_csv(paste(loc, "training-data/flux_modis_geo_daily.csv", sep = "")) %>% 
  dplyr::select(-Site.Name, -Data.Policy, -Site.Classification, -Upland.Class, -DOI.Reference, -Site.Personnel, -Base.File.Host,
                -`Upland Class`)
names(all_data)
```
...
###### Why is SWC, PPFD_IN and Upland Class logical?

```{r}
all_data %>% 
  group_by(ID) %>% 
  summarize(TAmin = min(TA, na.rm = T),
            SWCmin = min(SWC, na.rm = T),
            SWCmax = max(SWC, na.rm = T),
            PPFD_INmin = min(PPFD_IN, na.rm = T),
            PPFD_INmax = max(PPFD_IN, na.rm = T),
            Upland_Classmin = min(`Upland Class`, na.rm = T),
            Upland_Classmax = max(`Upland Class`, na.rm = T))
```

They all contain either `Inf` or `-Inf`.

###### Set all Inf or -Inf as NA

```{r}
all_data[all_data == Inf] <- NA
all_data[all_data == -Inf] <- NA
```

 
###### Remove extraneous variables and non-class-double data and impute all predictors
...
pp <- preProcess(all_data_impute, method = c("bagImpute"))
