...
data %>% 
  group_by(t) %>% 
  summarize(boot_mean = mean(sum_TgCH4month),
            boot_sd = sd(sum_TgCH4month)) %>% 
  ggplot(aes(t/12+2001, boot_mean)) +
  geom_line() + 
  geom_ribbon()
```
...
tgch4 <- nc_open(paste0(loc, "upscaling-output-raw/grids/upch4_v04_m1_TgCH4month_Aw_giems2.nc"))
...
write.csv(upscaling_sums, paste0(loc, "predictions/gridded/tg/sums_method1/", regions[i], "/upscaling_sums_giems2.csv"), row.names = F)
write.csv(annual_sums, paste0(loc, "predictions/gridded/tg/sums_method1/", regions[i], "/annual_sums_giems2.csv"), row.names = F)
write.csv(monthly_cycle, paste0(loc, "predictions/gridded/tg/sums_method1/", regions[i], "/monthly_cycle_giems2.csv"), row.names = F)
...
upscaling %>% 
  ...
  geom_line(aes(Dec_year, `CLASS-CTEM`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `DLEM_ech4_`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `ELM_ech4_c`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `JSBACH_ech`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `JULES_ech4`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `LPJ-GUESS_`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `LPJ-MPI_ec`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `LPJ-WSL_ec`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `LPX_ech4_c`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `ORCHIDEE_e`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `TEM-MDM_ec`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `TRIPLEX-GH`), color = 'grey70', alpha = 0.7) +
  geom_line(aes(Dec_year, `VISIT_ech4`), color = 'grey70', alpha = 0.7) +

   
    geom_ribbon(aes(ymin = sums_mean - sums_sd, ymax = sums_mean + sums_sd, fill = wetlands), alpha = 0.2) + # add observations
    geom_line(aes(color = wetlands)) +
  
    geom_line(aes(Dec_year, ctracker), color = "dark green", linetype = "F1") +
...
grid_sums %>% 
  group_by(Year) %>% 
  summarize_all(list(~sum(., na.rm = T))) %>% 
  dplyr::select(Year, Month, ctracker) %>% 
  filter(ctracker != 0) %>% 
  ungroup() %>% 
  summarize_all(list(~mean(., na.rm = T)))
...
grid_sums %>% 
  filter(!Year == 2018) %>% 
  gather(key = "gcp_model", value = "sum", 5:17) %>% 
  group_by(gcp_model) %>% 
  summarize(R2 = cor(sum, sums_mean)^2,
            MADiff = sum(abs(sums_mean - sum))/n(),
            Diff = mean(sum - sums_mean)) %>% 
  arrange(MADiff) %>% 
...
```{r}
# load global and regionanl comparison dataset
...
fig6_sums %>% 
  ...
  coord_flip() +
  my_theme
ggsave(paste0(loc, "predictions/gridded/tg/figures/hbl_comparison.png"),
