...
###### Read in final data and predictor metadata
...
<!-- ```{r} -->
<!-- pair_name <- paste("pair", 1:length(feat_pairs), sep = "") -->
<!-- fold_name <- paste("fold", 1:folds, sep = "") -->
<!-- file_name_df <- expand.grid(fold_name, pair_name)  -->
<!-- file_name <- paste(file_name_df$Var2, file_name_df$Var1, sep = "_") -->
<!-- ``` -->

##### Create a new list to match up the 4 lists

```{r}
train_list <- list()
for(i in 1:length(train_feat)){
  train_list[[i]] <- list(train_feat[[i]], train_label[[i]], feat_pairs[[1]], validate_data[[i]], folds_index[[i]])
  names(train_list[[i]]) <- c("train_feat", "train_label", "feat_pairs", "validate_data", "folds_index")
}
```

##### Write Function: Train multiple RFs (with CV) for FFS to select the first pair
...
##### Write Function: Input feature pair

```{r}

rf.pred.all.outer <- list()
...
fit_all_pairs(feat_pairs[[1]])

system.time(
  normal_2 <- lapply(feat_pairs[1:4], fit_all_pairs)
)

system.time(
mc_2 <- mclapply(feat_pairs[1:8], fit_all_pairs, mc.cores = numCores)
)
...
for(j in 1:10) {
  ...
  rf_metrics[[j]] <- ungroup(rf.pred.all) %>% 
    summarize(samples = n(),
              PMARE = 100/n() * sum(abs(FCH4 - FCH4P) / FCH4),
              R2 = cor(FCH4P, FCH4)^2,
              ...
              predictors = list(feat_pairs[[j]]) )

  print(paste(j, "th pair", sep = ""))
}
...
# rf_metrics_all <- rf_metrics_subset %>% bind_rows(rf_metrics_all)

# get length
length(rf_metrics_all$samples)

# look at metrics distribution
rf_metrics_all %>% 
  ggplot(aes(R2)) +
  geom_histogram() + mytheme
ggsave(paste("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/FFS/FWET_RELIEF/200820_ffs_subset_canopyht_R2.png", sep = ""),
       width = 8, height = 15, units = c("cm"), dpi = 300) 

rf_metrics_all %>% 
  ggplot(aes(MAE)) +
  geom_histogram() + mytheme
ggsave(paste("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/FFS/FWET_RELIEF/200820_ffs_subset_canopyht_MAE.png", sep = ""),
       width = 8, height = 15, units = c("cm"), dpi = 300) 
...
saveRDS(rf_metrics_all, "/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/FFS/FWET_RELIEF/200820_ffs_subset_metrics.rds")
