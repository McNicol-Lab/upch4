...
See: [Appendix: All Predictors](https://docs.google.com/spreadsheets/d/1DN0huLs-vVM3g_XcF1hBQrTpkKaGhzuWwaacfbe4iCo/edit#gid=0). Search for `EarthEnv` under column `Gridded Product`.
...
## Computed Predictors
computed <- read.csv("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Extractions/computed.csv")

# plot sin.yr
computed %>% 
  ggplot(aes(Month, sin.yr)) +
  geom_line() +
  facet_wrap(~ID) +
  mytheme
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Data QCQA/qaqc/sin.yr.pdf",
       width = 30, height = 30, units = c("cm"), dpi = 300)

# plot cos.yr
computed %>% 
  ggplot(aes(Month, cos.yr)) +
  geom_line() +
  facet_wrap(~ID) +
  mytheme
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Data QCQA/qaqc/cos.yr.pdf",
       width = 30, height = 30, units = c("cm"), dpi = 300)

# plot dec.yr
computed %>% 
  ggplot(aes(Month, dec.yr)) +
  geom_line() +
  facet_wrap(~ID) +
  mytheme
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Data QCQA/qaqc/dec.yr.pdf",
       width = 30, height = 30, units = c("cm"), dpi = 300)
...
computed %>% 
  ...
  facet_wrap(~ID) +
  mytheme
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Data QCQA/qaqc/Rpot.pdf",
       width = 30, height = 30, units = c("cm"), dpi = 300)


## WorldClim Predictors
wc <- read.csv("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Extractions/worldclim.csv")
names_generic <- paste(letters)[1:19]
wc <- wc %>% rename_at(5:23, ~names_generic)

# plot tempearture variables (wc1-11)
wc %>% 
  gather(key = wc_var, value = value, 5:23) %>% 
  mutate(group = ifelse(LAT > 60, "Boreal", NA),
         group = ifelse(LAT < 60 & LAT > 30, "Temperate", group),
         group = ifelse(LAT < 30, "Tropical", group)) %>% 
  filter(wc_var < "l") %>% 
    ggplot(aes(ID, value, color = group)) +
    geom_point() +
    facet_wrap(~wc_var, scales = 'free') +
    mytheme +
    theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Data QCQA/qaqc/wc.temp.pdf",
         width = 30, height = 30, units = c("cm"), dpi = 300)

# plot precip variables (wc12-19)
wc %>% 
  gather(key = wc_var, value = value, 5:23) %>% 
  mutate(group = ifelse(LAT > 60, "Boreal", NA),
         group = ifelse(LAT < 60 & LAT > 30, "Temperate", group),
  ...
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Data QCQA/qaqc/wc.precip.pdf",
       width = 30, height = 30, units = c("cm"), dpi = 300)
 

## EarthEnv Predictors
EE <- read.csv("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Extractions/earthenv.csv")

EE %>% 
  gather(key = earthenvpred, value = value, 5:length(EE)) %>%
  ggplot(aes(ID,value)) +
    geom_point() +
    facet_wrap(~earthenvpred, scales = 'free') +
  mytheme +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Figures/qaqc/earthenv_postqcqa.pdf",
       width = 50, height = 30, units = c("cm"), dpi = 300)


## Terraclimate
terraclimate <- read.csv("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Extractions/terraclimate.csv")

terraclimate %>% 
  gather(key = terraclimatepred, value = value, 7:length(terraclimate)) %>%
  filter(terraclimatepred == "aet") %>% 
  ...
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Figures/qaqc/aet.pdf",
       width = 50, height = 30, units = c("cm"), dpi = 300)

terraclimate %>% 
  gather(key = terraclimatepred, value = value, 7:length(terraclimate)) %>%
  filter(terraclimatepred == "soilwater") %>% 
  ggplot(aes(Month, value)) +
  geom_point() +
  facet_wrap(~ID, scales = 'free') +
  mytheme +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Figures/qaqc/soilwater.pdf",
       width = 50, height = 30, units = c("cm"), dpi = 300)

# seasonality
terraclimate %>% 
  gather(key = terraclimatepred, value = value, 7:length(terraclimate)) %>%
  filter(terraclimatepred == "aetS") %>% 
  ggplot(aes(Month, value)) +
  geom_point() +
  facet_wrap(~ID, scales = 'free') +
  mytheme +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Figures/qaqc/aetS.pdf",
       width = 50, height = 30, units = c("cm"), dpi = 300)

terraclimate %>% 
  gather(key = terraclimatepred, value = value, 7:length(terraclimate)) %>%
  filter(terraclimatepred == "soilwaterS") %>% 
  ggplot(aes(Month, value)) +
  geom_point() +
  facet_wrap(~ID, scales = 'free') +
  mytheme +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Figures/qaqc/soilwaterS.pdf",
       width = 50, height = 30, units = c("cm"), dpi = 300)
...
terraclimate %>% 
  ...
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Figures/qaqc/aetR.pdf",
       width = 50, height = 30, units = c("cm"), dpi = 300)

terraclimate %>% 
  gather(key = terraclimatepred, value = value, 7:length(terraclimate)) %>%
  filter(terraclimatepred == "soilwaterR") %>% 
  ggplot(aes(Year, value)) +
  geom_point() +
  facet_wrap(~ID, scales = 'free') +
  mytheme +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Figures/qaqc/soilwaterR.pdf",
       width = 50, height = 30, units = c("cm"), dpi = 300)
