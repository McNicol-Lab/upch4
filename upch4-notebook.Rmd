...
# global without USOWC
global_wo_usowc <- ungroup(rf.pred.all) %>% 
  # filter(!ID == "USOWC") %>%
  summarize(samples = n(),
            R2 = cor(FCH4P, FCH4)^2,
            NSE = 1 - sum((FCH4 - FCH4P)^2) / sum((FCH4 - mean(FCH4))^2),
...
sitemean_wo_usowc <- ungroup(rf.pred.all) %>% 
  group_by(ID) %>%
  filter(!ID == "USOWC") %>%
  summarize(MeanO = mean(FCH4, na.rm=TRUE),
            MeanP = mean(FCH4P, na.rm=TRUE),
...
write.csv(sitemean_wo_usowc,
          paste(loc, "predictions/cv/sitemean_wo_usowc.csv", sep = ""), 
          row.names = F)
...
### WITHOUT US-OWC
# by-ID
msc_wo_usowc <- ungroup(rf.pred.all) %>% 
  filter(!ID == "USOWC") %>% 
  group_by(ID, Month) %>%
  summarize(MeanO = mean(FCH4, na.rm=TRUE),
            MeanP = mean(FCH4P, na.rm=TRUE),
...
write.csv(msc_wo_usowc,
          paste(loc, "predictions/cv/msc_wo_usowc.csv", sep = ""), 
          row.names = F)
