...
ggsave(paste(loc, "predictions/cv/residuals_error.pdf", sep = ""),
       width = 13, height = 9, units = c("cm"), dpi = 300)
```
...
```{r}
data <- read.csv(paste(loc, "training-data/final.csv", sep = "")) %>% 
  dplyr::select(1:17, FCH4, FCH4_F_UNC, TA, TA_LAG2, canopyht, wc_mtdq,  wc_pwtm, EVI_LAG3, -Upscaling)
head(data)
```
...
static <- left_join(canopyht, worldclim) %>% 
  filter(Upscaling == 1) %>% 
  dplyr::select(-Upland.Class, -Upscaling)
names(static)
names(data)
```
...
```{r}
data <- data %>% 
  left_join(static, by = c("ID", "Site.Name", "Data.Policy", "Country", "Latitude", "Longitude", "IGBP",
                           "Site.Classification", "DOI", "DOI.Reference", "Site.Personnel", "Base.File.Host"))
```
...
data1 <- data %>% 
  rowwise() %>% 
  mutate(FCH4_mc = list(rnorm(1000, mean = FCH4, sd = FCH4_F_UNC)),
         TA_mc = list(rnorm(1000, mean = TA, sd = 1)),
         TA_LAG2_mc = list(rnorm(1000, mean = TA_LAG2, sd = 1)),
...
##### Now drop IDs at random from each of the 500 datasets

```{r}
site.names <- data3 %>% dplyr::select(ID) %>% filter(ID != "USOWC") %>% mutate(ID = factor(ID)) %>% pull() %>% levels()
site.drop <- sample(site.names, 500, replace = T)

data4 <- list()
for(i in 1:500){
  data4[[i]] <- data3 %>% 
    filter(mc_i == i) %>% 
    filter(ID != site.drop[i] & ID != "USOWC")
}
...
##### Identify which site was left out then subset those data  (forgot to set seed above)

```{r warning = F, message = F, echo = F}
# convert ID vector to column
ID.names <- site.names %>% 
  as_tibble() %>% 
  rename("ID" = "value")
...
##### Finally, subset the single held-out site from each mc_i

```{r}
data6 <- list()
for(i in 1:500){
  data6[[i]] <- data3 %>% 
    filter(mc_i == i & ID == ID.drop[i])
...
```{r}
raster.names <-list.files(paste(loc, "grids/merra2/", sep = ""), pattern = "nc4$", full.names = F)
nc_data <- lapply(paste(loc, "grids/merrra2/", raster.names, sep = ""), nc_open)
```
...
```{r}
{
  sink('merra2_2001-2015_metadata.txt')
  print(nc_data)
  sink()
}
```
...
# read MODIS
# EVI
loc <- "/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Grids/MODIS/EVI/"
setwd(loc)
raster.names <-list.files(loc, pattern = "tif$", full.names = FALSE)
evi <- stack(raster.names)
evi_names <- paste(substr(names(evi),2,5),0,substr(names(evi),6,6),sep="")
for(i in 1:nlayers(evi)){
  writeRaster(evi[[i]],  paste("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Grids/MODIS/EVI/long names/",evi_names[i],".tif",sep=""), overwrite = TRUE)
}
...
evi_res <- list()
for(i in 1:length(years)){
  evi_res[[i]] <- resample(evi[[ years[[i]] ]], cos.yr)
}
endCluster()
end_time <- Sys.time()
end_time - start_time

evi_res <- stack(evi_res)

# get annual min for all 18 years
evi_min <- list()
for(i in 1:18){
  evi_min[[i]] <- min(evi_res[[ years[[i]] ]], na.rm=TRUE)
}

evi_min <- stack(evi_min)

writeRaster(evi_min, "/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Grids/final_ffs/EVI_min.nc", format="CDF", overwrite=TRUE)

## load stack
evi_min <- stack("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Grids/final_ffs/EVI_min.nc")
EVI_overall_min <- mean(evi_min, na.rm=TRUE)
writeRaster(EVI_overall_min, "/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Grids/final_ffs/dissimilarity_only/EVI_overall_min.tif", overwrite=TRUE)
