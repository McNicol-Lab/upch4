...
```
##### Stepwise FFS (for first 20 steps, to avoid local minima)
...
##### Train multiple RFs (with CV) for FFS to select the next 20 best predictors
...
for(i in 1:feat_l){
  write.csv(rf_pred[[i]], paste(loc, "ffs/stepwise/rf-pred/rf_pred_", set_index[i], "_", feat[i],  ".csv", sep = ""), row.names = F)
}
