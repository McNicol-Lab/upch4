...
write.csv(flux_modis_geo_daily, paste(loc, "/training-data/flux_modis_geo_daily.csv", sep = ""))
...
cluster_bycluster <- cluster_bysite %>% ungroup() %>% 
  ...
  summarize(Class = Class[1],
            ...
            count = sum(count),
            percent = count/total[1]*100) %>% 
  mutate(Band = ifelse(Lat < 30, "Tropics",NA),
         Band = ifelse(Lat < 60 & Lat > 30, "Temperate", Band),
         Band = ifelse(Lat > 60, "Boreal", Band)) 

# output cluster stats
cluster_bycluster %>% 
  write.csv(., "/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Clusters/SOIL/200709_cluster_stats.csv",
            row.names=FALSE)
```
