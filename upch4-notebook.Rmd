...
```{r}
data <- read.csv(paste(loc, "training-data/final.csv", sep = "")) %>% 
  dplyr::select(Cluster, ID, Year, Month, Week, FCH4, FCH4_F_UNC, TA, TA_LAG2, canopyht, wc_mtdq,  wc_pwtm, EVI_LAG3)
head(data)
```
...
tgrid <- expand.grid(
  .mtry = c(2,4,6),
  .splitrule = "variance", 
  .min.node.size = c(iteration_l*0.001,iteration_l*0.01)
)
...
tune_MAEse <- list()
for (i in 1:length(rf_model)){
  mean_MAE[i] <- rf_model[[i]]$results$MAE[which(rf_model[[i]]$results$MAE == min(rf_model[[i]]$results$MAE))] 
  mean_r2[i] <- rf_model[[i]]$finalModel$r.squared
  tune_r2[[i]] <- rf_model[[i]]$results[,c(5)]
  tune_MAE[[i]] <- rf_model[[i]]$results[,c(4)]
  tune_r2se[[i]] <- rf_model[[i]]$results[,c(6)]
  tune_MAEse[[i]] <- rf_model[[i]]$results[,c(4)]

}
summary(mean_r2) # oob R2
...
rf.pred.all %>% 
  ...
  ylab(label = expression("FCH4P (nmol m"^{-2}*" s"^{-1}*")")) +
  mytheme
ggsave(paste("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Monte Carlo/predictions/201009_rf_mc_12b_nocanopyht.png", sep = ""),
       width = 24, height = 24, units = c("cm"), dpi = 300)

# Drop bias correction because seems to have very minor effect and not easy to grid
# try bias correction
splinemod <- smooth.spline(y=rf.pred.all$FCH4, x=rf.pred.all$FCH4P, spar = 2)
# plot(rf.pred.all$FCH4P[1:10000],rf.pred.all$FCH4[1:10000], xlim = c(0,500), ylim = c(0,500))
# abline(0,1,col = 'blue')
# lines(splinemod, col="red", lwd=3)

saveRDS(splinemod, "/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Monte Carlo/predictions/201009_rf_mc_spline_nocanopyht.rds")

# get bias corrected predictions
FCH4P_cor <- predict(splinemod, rf.pred.all$FCH4P)
FCH4P_cor <- FCH4P_cor$y
rf.pred.all <- cbind(rf.pred.all, FCH4P_cor)

# look at pred vs obs
rf.pred.all %>% 
  filter(mc_i %in% c(1:12)) %>%
  ggplot(aes(FCH4, FCH4P_cor)) +
  # stat_binhex(aes(fill=log(..count..))) +
  geom_point() +
  geom_abline(slope = 1, color = 'white') +
  facet_wrap(~mc_i) +
  ...
  # scale_y_continuous(limits = c(0,1000)) +
  xlab(label = expression("FCH4 (nmol m"^{-2}*" s"^{-1}*")")) +
  ylab(label = expression("FCH4P Bias Cor. (nmol m"^{-2}*" s"^{-1}*")")) +
  mytheme
ggsave(paste("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Monte Carlo/predictions/201009_rf_mc_12b_bias_cor_nocanopyht.png", sep = ""),
       width = 24, height = 24, units = c("cm"), dpi = 300)

write.csv(rf.pred.all, "/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Monte Carlo/predictions/201009_rf_mc_pred_nocanopyht.csv",
          row.names = FALSE)

# experiment and date
tune_r2s %>% 
  ggplot(aes(min.node.size, R2, col = factor(mtry))) +
  geom_line() +
  scale_x_log10() +
  facet_wrap(~fold) +
  mytheme
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Monte Carlo/predictions/tune_r2.pdf",
       width = 20, height = 20, units = c("cm"), dpi = 300)

tune_MAEs %>% 
  ggplot(aes(min.node.size, MAE, col = factor(mtry))) +
  geom_line() +
  scale_x_log10() +
  facet_wrap(~fold) +
  mytheme
ggsave("/Volumes/LACIE SHARE/Stanford CH4/June 2020 Upscaling/Monte Carlo/predictions/tune_mae.pdf",
       width = 20, height = 20, units = c("cm"), dpi = 300)
