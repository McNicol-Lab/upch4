...
```{r}
all_data %>% 
   dplyr::select(-Cluster, -`Site.Name`, -`Data.Policy`, -Country, -IGBP, -`Site.Classification`, -DOI,  -`DOI.Reference`, -`Site.Personnel`, 
...
all_data_impute <- all_data %>% 
  dplyr::select(-Cluster, -ID, -`Site.Name`, -`Data.Policy`, -Country, -IGBP, -`Site.Classification`, -DOI,  -`DOI.Reference`, -`Site.Personnel`, 
                -`Upland.Class`, -`Base.File.Host`, -Upscaling, -FCH4, -FCH4_F_UNC, -imputed) 
...
all_data <- all_data %>% 
  ...
  mutate(rpot_LAG4 = lag(rpot, 4, default = median(rpot[1:4], na.rm=TRUE)), ## generic monthly seasonality (4,8,12)
         ...
         TA_LAG1 = lag(TA, 1, default = median(TA[1:1], na.rm=TRUE)),  ## MET daily seasonality (1,4)
         TA_LAG4 = lag(TA, 4, default = median(TA[1:4], na.rm=TRUE)), 
         TA_LEAD1 = lead(TA, 1, default = median(TA[(length(TA)-1):length(TA)], na.rm=TRUE)), 
         TA_LEAD4 = lead(TA, 4, default = median(TA[(length(TA)-4):length(TA)], na.rm=TRUE)), 
         
         SW_IN_LAG1 = lag(SW_IN, 1, default = median(SW_IN[1:1], na.rm=TRUE)),  
         SW_IN_LAG4 = lag(SW_IN, 4, default = median(SW_IN[1:4], na.rm=TRUE)), 
         SW_IN_LEAD1 = lead(SW_IN, 1, default = median(SW_IN[(length(SW_IN)-1):length(SW_IN)], na.rm=TRUE)), 
         SW_IN_LEAD4 = lead(SW_IN, 4, default = median(SW_IN[(length(SW_IN)-4):length(SW_IN)], na.rm=TRUE)), 
         
         LW_IN_LAG1 = lag(LW_IN, 1, default = median(LW_IN[1:1], na.rm=TRUE)), 
         LW_IN_LAG4 = lag(LW_IN, 4, default = median(LW_IN[1:4], na.rm=TRUE)), 
         LW_IN_LEAD1 = lead(LW_IN, 1, default = median(LW_IN[(length(LW_IN)-1):length(LW_IN)], na.rm=TRUE)), 
         LW_IN_LEAD4 = lead(LW_IN, 4, default = median(LW_IN[(length(LW_IN)-4):length(LW_IN)], na.rm=TRUE)), 
         
         PA_LAG1 = lag(PA, 1, default = median(PA[1:1], na.rm=TRUE)),  
         PA_LAG4 = lag(PA, 4, default = median(PA[1:4], na.rm=TRUE)), 
         PA_LEAD1 = lead(PA, 1, default = median(PA[(length(PA)-1):length(PA)], na.rm=TRUE)), 
         PA_LEAD4 = lead(PA, 4, default = median(PA[(length(PA)-4):length(PA)], na.rm=TRUE)), 
         
         VPD_LAG1 = lag(VPD, 1, default = median(VPD[1:1], na.rm=TRUE)),  
         VPD_LAG4 = lag(VPD, 4, default = median(VPD[1:4], na.rm=TRUE)), 
         VPD_LEAD1 = lead(VPD, 1, default = median(VPD[(length(VPD)-1):length(VPD)], na.rm=TRUE)), 
         VPD_LEAD4 = lead(VPD, 4, default = median(VPD[(length(VPD)-4):length(VPD)], na.rm=TRUE)), 
         
         P_LAG1 = lag(P, 1, default = median(P[1:1], na.rm=TRUE)),  
         P_LAG4 = lag(P, 4, default = median(P[1:4], na.rm=TRUE)), 
         P_LEAD1 = lead(P, 1, default = median(P[(length(P)-1):length(P)], na.rm=TRUE)), 
         P_LEAD4 = lead(P, 4, default = median(P[(length(P)-4):length(P)], na.rm=TRUE)), 
         
         
         NEE_LAG1 = lag(NEE, 1, default = median(NEE[1:1], na.rm=TRUE)),  ## FLUX daily seasonality (1,4,4)
         NEE_LAG4 = lag(NEE, 4, default = median(NEE[1:4], na.rm=TRUE)), 
         NEE_LEAD1 = lead(NEE, 1, default = median(NEE[(length(NEE)-1):length(NEE)], na.rm=TRUE)), 
         NEE_LEAD4 = lead(NEE, 4, default = median(NEE[(length(NEE)-4):length(NEE)], na.rm=TRUE)), 
         
         GPP_LAG1 = lag(GPP, 1, default = median(GPP[1:1], na.rm=TRUE)),  
         GPP_LAG4 = lag(GPP, 4, default = median(GPP[1:4], na.rm=TRUE)), 
         GPP_LEAD1 = lead(GPP, 1, default = median(GPP[(length(GPP)-1):length(GPP)], na.rm=TRUE)), 
         GPP_LEAD4 = lead(GPP, 4, default = median(GPP[(length(GPP)-4):length(GPP)], na.rm=TRUE)), 
         
         RECO_LAG1 = lag(RECO, 1, default = median(RECO[1:1], na.rm=TRUE)),  
         RECO_LAG4 = lag(RECO, 4, default = median(RECO[1:4], na.rm=TRUE)), 
         RECO_LEAD1 = lead(RECO, 1, default = median(RECO[(length(RECO)-1):length(RECO)], na.rm=TRUE)), 
         RECO_LEAD4 = lead(RECO, 4, default = median(RECO[(length(RECO)-4):length(RECO)], na.rm=TRUE)), 
         
         LE_LAG1 = lag(LE, 1, default = median(LE[1:1], na.rm=TRUE)),  
         LE_LAG4 = lag(LE, 4, default = median(LE[1:4], na.rm=TRUE)), 
         LE_LEAD1 = lead(LE, 1, default = median(LE[(length(LE)-1):length(LE)], na.rm=TRUE)), 
         LE_LEAD4 = lead(LE, 4, default = median(LE[(length(LE)-4):length(LE)], na.rm=TRUE))) %>% 
  ungroup()
```
...
```{r}
names(all_data)[24:146] %>% write.csv(paste(loc, "/training-data/list_of_predictors.csv", sep = ""))
```
...
write.csv(cluster_bysite, paste(loc, "/training-data/untitled folder/cluster_bysite.csv", sep = ""), row.names = F)
write.csv(cluster_bycluster, paste(loc, "/training-data/untitled folder/cluster_bycluster.csv", sep = ""), row.names = F)
```
