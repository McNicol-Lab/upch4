...
## Cross Validation (Train, Predict)
...
saveRDS(rf_model, paste("ensembles/cv/best6.rds", sep = "")) # local dir.
```
...
as_tibble(cbind(metrics, values)) %>% 
  write_csv(paste(loc, "ensembles/cv/best6_cv_metrics.csv", sep = ""),
            row.names = F)
```
...
```{r}
rf.pred.all <- read_csv(paste(loc, "predictions/cv/best6_pred.csv", sep = ""))
```
...
write_csv(global, paste(loc, "predictions/cv/global.csv", sep = ""),
          row.names = F)
...
write_csv(sitemean_data, paste(loc, "predictions/cv/sitemean.csv", sep = ""),
          row.names = F)
...
# join sitemean
sitemean_data <- bind_rows(sitemean, sitemean_class, sitemean_climate, sitemean_id)
write_csv(sitemean_data, paste(loc, "predictions/cv/sitemean_wo_usowc.csv", sep = ""),
          row.names = F)
...
sitemean_data <- read_csv(paste0(loc, "predictions/cv/sitemean_wo_usowc.csv"))
...
write_csv(msc_data, paste(loc, "predictions/cv/msc.csv", sep = ""),
          row.names = F)
...
# join msc
msc_data <- bind_rows(msc, msc_class, msc_climate, msc_id)
write_csv(msc_data, paste(loc, "predictions/cv/msc_wo_usowc.csv", sep = ""),
          row.names = F)
...
write_csv(anomaly_data, paste(loc, "predictions/cv/anomaly.csv", sep=""),
          row.names=FALSE)
