...
```{r read-static-pred-mc-data, message = F}
canopyht <- read_csv("mc/extracted/canopyht_sd.csv")
worldclim <- read_csv("mc/extracted/worldclim_sd.csv")
...
```{r generate-datasets-with-mc, warning = F, echo = F, message = F}
set.seed(23)
...
data2 <- data %>% 
  ...
  rowwise() %>% 
  mutate(FCH4_mc = list(rlaplace(1000, mean = FCH4, sd = FCH4_UNC/2)),
...
```{r drop-random-clusters}
set.seed(23)
cluster_drop <- sample(1:26, 500, replace = T)
write_csv(as_tibble(cluster_drop), "mc/data/cluster_drop.csv")

data5 <- list()
for(i in 1:500){
  data5[[i]] <- data4 %>% 
    filter(mc_i == i) %>% 
    filter(Cluster != cluster_drop[i] & ID != "USOWC")
}
...
saveRDS(data6, 
        "mc/data/1000mc_500bs.rds")
```

Identify which site was left out then subset those data

```{r get-dropped-ids, warning = F, message = F, echo = F}
...
saveRDS(data7, 
        "mc/data/1000mc_500bs_heldout.rds")
```

To train the full ensemble, repeat random forest sections in 2. for each of the 500 boostrap sampled MC-simulated datasets.
