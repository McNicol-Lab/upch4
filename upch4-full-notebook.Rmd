...
ggsave(paste(loc, "/training-data/imputation-qc/weekly/TA.png", sep = ""),
               width = 30, height = 40, units = c("cm"), dpi = 300)
...
```
 ...
 What is the distribution of imputed %s?

```{r}
all_data %>% 
  mutate(exclude = ifelse(imputed > 0.857, "yes", "no")) %>% 
  ggplot(aes(imputed, fill = exclude, color = exclude)) +
  geom_histogram(alpha = 0.2) +
  geom_vline(xintercept = 0.857) +
  labs(x = "Imputed fraction", y = "Number of weeks") +
  my_theme
ggsave(paste0(loc, "training-data/missingness/fch4-gap-distribution.png"),
       width = 15, height = 10, units = c("cm"), dpi = 900)
```

 Subset only weeks with at least one day (14%) of observations
...
# # get monthly wetland area 
# wad2m_global_area <- cellStats(stack, sum)
# wad2m_global_area %>% 
#   unname() %>% 
#   as_tibble() %>% 
#   rename(area = value) %>% 
#   mutate(timestep = 1:length(wad2m_global_area)) %>% 
#   write.csv(paste0(loc, "/training-data/clusters/wad2m-area.csv"))
...
# get colors
col_vector <-  c("#E78AC3","orange","#33A02C","#377EB8","#E41A1C","#999999")
...
```
 ...
 Summarize/visualize the temporal distribution of data

```{r}
all_data <- read.csv(paste0(loc, "training-data/final.csv"))

head(all_data)
all_data %>% 
  group_by(Year) %>% 
  summarize(count = n(),
            percentage = count/length(all_data$Cluster)*100) %>% 
  write.csv(paste0(loc, "/training-data/temporal_distribution.csv"))
```
...
```
 ...
 Northern latitude metrics

For comparison to Peltola et al. (2019)
```{r}
...
sitemean_data <- read.csv(paste0(loc, "predictions/cv/sitemean_wo_usowc.csv"))
...
rf.pred.all %>% 
  ...
  my_theme
ggsave("/Volumes/Samsung_T5/Stanford/upch4_local/figures-writing/GBC/SI Figures/hysteresis.png", height = 10, width = 8)
```
...
write.csv(cluster.drop, paste0(loc, "mc/data/cluster.drop.csv"), row.names = F)
...
```{r warning = F, message = F, echo = F}
cluster.drop <- read.csv(paste0(loc, "mc/data/cluster.drop.csv")) %>% pull()
...
#### 9. (cont.) Partial Dependency Plots for MC

They look reasonable.

```{r}
# read mc rf model
rf_model <- readRDS(paste0(loc, "ensembles/mc/rf.rds"))

# Read mc training data csv
train <- readRDS(paste0(loc, "mc/data/1000mc_500bs.rds")) # local path

# read folds index
folds_index <- readRDS(paste0(loc, "mc/data/folds_index.rds")) # local path

# pdp
ice.TA <- rf_model[[1]] %>% 
  partial(pred.var = c("wc_pwtm_mc"), train = train[folds_index[[1]][[1]],], plot.engine = "ggplot2", ice = FALSE, center = TRUE)
plotPartial(ice.TA, colorkey= TRUE) 
...
summary %>% 
  ...
  geom_point(color = "blue", alpha = 0.7, size = 0.5) + 
...
summary %>% 
  ...
  ggplot(aes(year_dec, meanFCH4P)) + 
  geom_errorbar(aes(year_dec, ymin = meanFCH4P - 2*sdFCH4P, ymax = meanFCH4P + 2*sdFCH4P), alpha = 0.9, color = 'grey') +
  geom_point(color = "blue", alpha = 0.7, size = 0.3) + 
  geom_point(aes(year_dec, meanFCH4), size = 0.5) +
...
```{r}
all_stack_weighted_full <- stack(paste0(loc, "dissimilarity/output/stacks/all_weighted.nc"))
month_vector_st <- c(seq(1,72,6))
month_vector_end <- month_vector_st+5
...
for (i in 1:12){ # months
  ...
  ggplot() +
    ...
    coord_equal() +
    scale_fill_gradient2(low = "white", mid = 'black', high = "black", midpoint = all_sites_DI_AOA) +
    scale_y_continuous(limits=c(-60, 85)) +
    labs(title = "Global Dissimilarity Index from 43 FLUXNET-CH4 Wetlands") +
    theme_map(8) +
    theme(legend.position = 'right')

  ggsave(file = paste("all_DI_aoa_newramp_",i,".png", sep=""),
...
```{r}
monthly_global_DI <- read.csv(paste0(loc, "dissimilarity/output/stats/all_monthly_global_DI_subset.csv"))
```
...
```{r}
monthly_global_DI_plot <- monthly_global_DI %>% 
  dplyr::select(closest_tower, DI) %>% 
...
summary(lm(monthly_global_DI_plot$DI ~ monthly_global_DI_plot$cover))
  
monthly_global_DI_plot %>% 
  ggplot(aes(cover, DI)) + 
  geom_point() +
...
summary(lm(di_curve$var ~ di_curve$DI)) # get p value
...
```{r}
site_month_DI_metrics <- site_month_DI  %>% 
  filter(ID != "USOWC") %>% 
  dplyr::select(ID, Month, min_dist, mean_cvCluster_dist, DI) %>% 
  right_join(site_month_metrics, by = c("ID", "Month"))

summary(lm(site_month_DI_metrics$MAE ~ site_month_DI_metrics$DI)) # get p value


site_month_DI_metrics %>% 
  filter(ID != "USOWC") %>% 
  dplyr::select(ID, Month, min_dist, mean_cvCluster_dist, DI) %>% 
  right_join(site_month_metrics, by = c("ID", "Month")) %>% 
  ggplot(aes(DI, MAE)) +
...
for (i in 1:216){
  upscaling_stack[[i]]<- stack(TA[[i]], TA_LAG2[[i]], EVI_LAG3[[i]], wc_mtdq, wc_pwtm, canopyht) 
  names(upscaling_stack[[i]]) <- c( "TA", "TA_LAG2", "EVI_LAG3", "wc_mtdq", "wc_pwtm", "canopyht")
}
...
```{r}
# test running one rf model on first timestep

forcing_data <- stack("/Volumes/Samsung_T5/Stanford/upch4_local/grids/ffs/forcing/forcing_001.nc")
ts1 <- predict(forcing_data, rf_model[[1]])
...
# check the overall 2001-2018 mean global emission = 146.616 Tg CH4 y-1 (which matches following method 1)
data %>% 
  mutate(year = rep(2001:2018, each = 500*12)) %>% 
  group_by(year,b) %>% 
  summarize(yearly_sum = sum(sum_TgCH4month)) %>% 
  group_by(year) %>% 
  summarize(boot_mean = mean(yearly_sum)) %>% pull(boot_mean) %>% mean()
```


 Make map of nmol predictions (unweighted)

```{r}
# read in nmol flux map - unweighted by wetland area
...
# set all pixels corresponding to WAD2M august NAs to NA throughout the nmol stack
for(i in 1:nlayers(nmol)){
  nmol[[i]][is.na(max)] <- NA
...
# convert to spatial dataframe
nmol_spdf <- as(nmol, "SpatialPixelsDataFrame")
head(nmol_spdf)
nmol_df <- as.data.frame(nmol_spdf) %>% 
  gather(key = "timestep", value = "ch4_flux_nmol", 1:216) 
head(nmol_df)

# compute mean of "at least some wetland area" cover
nmol_df %>% 
  dplyr::select(ch4_flux_nmol) %>% 
  ggplot(aes(ch4_flux_nmol)) +
  geom_histogram() +
  my_theme +
  labs(x = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')'), y = "Count")
ggsave(paste0(loc, "predictions/gridded/nmol/figures/nmol_global_distribution.png"), 
              width = 15, height = 12, units = c("cm"), dpi = 300)

# # assign all NAs to zero so that we can compute a mean flux across all grids
nmol[is.na(nmol)] <- 0

# compute mean raster for a figure
nmol_mean <- mean(nmol)

# remove all zero wetland cover
nmol_mean[is.na(max)] <- NA

plot(nmol_mean)



coords <- c(-150, 0) %>% matrix(ncol = 2)

test <- extract(nmol[[7]], coords)
...
nmol <- nc_open(paste0(loc, "upscaling-output-raw/grids/upch4_v04_m1_nmolm2sec.nc"))
  ...
  # gather into long files
  months <- rep(c(rep(1:12, each = 8)), 18) # 18 years x 12 months x 8 sites
  years <- c(rep(2001:2018, each = 12*8)) # 
  length(months) == length(years)
...
```
 ...
 Compare grid predictions with some training data

```{r}
# get predictions
site.pred <- read.csv(paste0(loc, "predictions/gridded/nmol/training-site-pred.csv"))

# select what we need
site.pred <- site.pred %>% 
  mutate(ID = paste0(substr(ID, 1,2), substr(ID, 4, 6))) %>% 
  dplyr::select(Class = Site.Classification, ID, Year, Month, mean_FCH4P = mean_FCH4P_grid_nmol, sd_FCH4P = sd_FCH4P_grid_nmol) %>% 
  group_by(Class, ID, Year, Month) %>% 
  summarize(mean_FCH4P = mean(mean_FCH4P, na.rm=T),
            sd_FCH4P = mean(sd_FCH4P, na.rm=T)) 

# get training data
site.data <- read.csv(paste0(loc, "training-data/final.csv")) %>% 
  dplyr::select(ID, Year, Month, FCH4, FCH4_UNC) %>% 
  group_by(ID, Year, Month) %>% 
  summarize(mean_FCH4 = mean(FCH4, na.rm = T),
            sd_FCH4 = mean(FCH4_UNC, na.rm = T)) 

# join
site.all <- site.pred %>% left_join(site.data)
...
# global metrics
ungroup(site.all) %>% 
  filter(!is.na(mean_FCH4) & !is.na(mean_FCH4P)) %>% 
  # filter(!ID == "USOWC") %>%
  summarize(samples = n(),
            R2 = cor(mean_FCH4P, mean_FCH4)^2,
            NSE = 1 - sum((mean_FCH4 - mean_FCH4P)^2) / sum((mean_FCH4 - mean(mean_FCH4))^2),
            MAE = sum(abs(mean_FCH4 - mean_FCH4P))/n(),
            nMAE = MAE/sd(mean_FCH4),
            MeanO = mean(mean_FCH4),
            MedO = median(mean_FCH4),
            sdO = sd(mean_FCH4),
            MeanP = mean(mean_FCH4P),
            MedP = median(mean_FCH4P),
            sdP = sd(mean_FCH4P),
            nSD = sdP/sdO,
            Bias = mean(mean_FCH4P - mean_FCH4),
            cBias = abs(Bias)/sum(abs(Bias))*100)
```
...
site.all %>% 
  group_by(ID) %>% 
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(Class %in% c("Bog"), !ID == "SESt1",
         Year_dec > 2011 & Year_dec < 2017) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2011:2016), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  geom_line(aes(y = mean_FCH4P, color = "dark blue"), size = 0.5, alpha = 0.7) +
  facet_wrap(~ID, ncol= 2, scales = "free") +
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/bogs.png"),
       width = 20, height = 18, units = c("cm"), dpi = 300)
```

 Interesting bog - NZ-Kop
...
site.all %>% 
  ...
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(ID == "NZKop",
         Year_dec > 2012 & Year_dec < 2016) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2013:2015), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/nz-kop.png"),
       width = 14, height = 10, units = c("cm"), dpi = 300)
```
...
site.all %>% 
  ...
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(Class %in% c("Fen"), !ID == "RUVrk",
         Year_dec > 2006 & Year_dec < 2019) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2007:2018), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/fens.png"),
       width = 20, height = 18, units = c("cm"), dpi = 300)
```

 Interesting fen - FI-Lom
...
site.all %>% 
  ...
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(ID == "DEHte",
         Year_dec > 2011 & Year_dec < 2018) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2012:2017), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/de-hte.png"),
       width = 14, height = 10, units = c("cm"), dpi = 300)
```
...
site.all %>% 
  ...
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(Class %in% c("Marsh"), !ID == "RUVrk",
         Year_dec > 2010 & Year_dec < 2019) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2011:2017), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/marshes.png"),
       width = 20, height = 22, units = c("cm"), dpi = 300)
```

 Interesting marsh - US-Myb
...
site.all %>% 
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/us-myb.png"),
       width = 14, height = 10, units = c("cm"), dpi = 300)
```
...
site.all %>% 
  ...
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(Class %in% c("Wet tundra"), !ID == "RUVrk",
         Year_dec > 2009 & Year_dec < 2017) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2010:2016), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/wet_tundra.png"),
       width = 20, height = 24, units = c("cm"), dpi = 300)
```

 Interesting wet tunrda - US-Ivo
...
site.all %>% 
  ...
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(ID == "USIvo",
         Year_dec > 2013 & Year_dec < 2017) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2014:2016), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/us-ivo.png"),
       width = 14, height = 10, units = c("cm"), dpi = 300)
```
...
site.all %>% 
  ...
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(Class %in% c("Swamp"), !ID == "RUVrk",
         Year_dec > 2012 & Year_dec < 2019) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2013:2017), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/swamp.png"),
       width = 20, height = 18, units = c("cm"), dpi = 300)
```

 Interesting swamp 1 - MY-MLM
...
site.all %>% 
  ...
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(ID == "MYMLM",
         Year_dec > 2014 & Year_dec < 2016) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2015:2015), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/my-mlm.png"),
       width = 14, height = 10, units = c("cm"), dpi = 300)
```

 Interesting swamp 2 - US-NC4
...
site.all %>% 
  ...
  mutate(Month_dec = Month/12,
         Year_dec = Year+Month_dec) %>% 
  filter(ID == "USNC4",
         Year_dec > 2012 & Year_dec < 2017) %>%
  ggplot(aes(x = Year_dec)) +
  geom_vline(xintercept = c(2013:2016), color = 'grey70', linetype = 'dashed') +
  geom_ribbon(aes(ymin = mean_FCH4 - sd_FCH4, ymax = mean_FCH4 + sd_FCH4), fill =  "grey70", alpha = 0.3) + # add observations
  geom_line(aes(y = mean_FCH4), size = 0.5) +
  geom_point(aes(y = mean_FCH4), shape = 21, fill = "white") +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), fill =  "grey70", alpha = 0.3) + # add predictions
  ...
  my_theme +
  labs(x= "Year", y = expression(CH[4]*' Flux (nmol m'^{-2}*' s'^{-1}*')')) 
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/us-nc4.png"),
       width = 14, height = 10, units = c("cm"), dpi = 300)
```

 Compare grid predictions with test (AmeriFlux validation) data
...
PEQFR <- read.csv(paste0(loc, "fluxnet-ch4-data/ameriflux-validation/csv/AMF_PE-QFR_BASE_HH_1-5.csv"), skip = 2) %>%
  mutate(ID = "PE-QFR",
         ...
         Week = ifelse(Week == 53, 52, Week)) %>% 
  group_by(Month) %>% 
  summarize(FCH4_mean = mean(FCH4, na.rm = TRUE),
            FCH4_25 = quantile(FCH4, probs = 0.25, na.rm = TRUE),
            FCH4_75 = quantile(FCH4, probs = 0.75, na.rm = TRUE),) 

site.pred %>% 
  filter(ID == "PE-QFR") %>% 
  left_join(PEQFR) %>% 
  ggplot(aes(Month, FCH4_mean)) + 
  # geom_line(aes(y = FCH4), size = 0.5) +
  geom_point(shape = 21, fill = "white") +
  geom_errorbar(aes(ymin = FCH4_25, ymax = FCH4_75), width = 0, alpha = 0.5) +
...
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/PE-QFR.png"),
       width = 14, height = 12, units = c("cm"), dpi = 300)

# get stats
joined_qfr <- site.pred %>% 
  filter(ID == "PE-QFR") %>% 
  left_join(PEQFR)  %>% 
  filter(!is.na(FCH4))

cor(joined_qfr$mean_FCH4P, joined_qfr$FCH4)^2 # r2
sum(abs(joined_qfr$mean_FCH4P - joined_qfr$FCH4))/length(joined_qfr$mean_FCH4P) # mae
sum(abs(joined_qfr$mean_FCH4P - joined_qfr$FCH4))/length(joined_qfr$mean_FCH4P)/sd(joined_qfr$FCH4) # nmae
sum(joined_qfr$mean_FCH4P - joined_qfr$FCH4)/length(joined_qfr$mean_FCH4P) # bias
length(joined_qfr$mean_FCH4P)
```
...
site.pred %>% 
  ...
  # geom_line(aes(y = FCH4), size = 0.5) +
  geom_point(shape = 21, fill = "white") +
  geom_errorbar(aes(ymin = FCH4_mean - FCH4_sd, ymax = FCH4_mean + FCH4_sd), width = 0, alpha = 0.5) +
  geom_line(aes(Month, mean_FCH4P)) +
  geom_ribbon(aes(ymin = mean_FCH4P - sd_FCH4P, ymax = mean_FCH4P + sd_FCH4P), alpha = 0.1) + # add predictions
  scale_color_manual(values = rep(c("dark blue", "dark red"), 4)) +
  scale_fill_manual(values = rep("grey70", 8)) +
  ...
  # scale_y_continuous(limits = c(0, 100)) +
  scale_x_continuous(breaks = c(1:12), limits = c(1,12))
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/US-ALQ.png"),
       width = 14, height = 12, units = c("cm"), dpi = 300)

# get stats
joined_alq <- site.pred %>% 
  filter(ID == "US-ALQ") %>% 
  left_join(USALQ)  %>% 
  filter(!is.na(FCH4))

cor(joined_alq$mean_FCH4P, joined_alq$FCH4)^2 #r2
sum(abs(joined_alq$mean_FCH4P - joined_alq$FCH4))/length(joined_alq$mean_FCH4P) #mae
sum(abs(joined_alq$mean_FCH4P - joined_alq$FCH4))/length(joined_alq$mean_FCH4P)/sd(joined_alq$FCH4) # nmae
sum(joined_alq$mean_FCH4P - joined_alq$FCH4)/length(joined_alq$mean_FCH4P) #bias
length(joined_alq$mean_FCH4P)
```
...
site.pred %>% 
  ...
  scale_y_continuous(limits = c(0, 100)) +
  scale_x_continuous(breaks = c(1:12), limits = c(1,12))
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/CA-CF2.png"),
       width = 14, height = 12, units = c("cm"), dpi = 300)

# get stats
joined_cf2 <- site.pred %>% 
  filter(ID == "CA-CF2") %>% 
  left_join(CACF2)  %>% 
  filter(!is.na(FCH4))

cor(joined_cf2$mean_FCH4P, joined_cf2$FCH4)^2
sum(abs(joined_cf2$mean_FCH4P - joined_cf2$FCH4))/length(joined_cf2$mean_FCH4P)
sum(abs(joined_cf2$mean_FCH4P - joined_cf2$FCH4))/length(joined_cf2$mean_FCH4P)/sd(joined_cf2$FCH4) # nmae
sum(joined_cf2$mean_FCH4P - joined_cf2$FCH4)/length(joined_cf2$mean_FCH4P)
length(joined_cf2$mean_FCH4P)
```
...
```{r}
# read in site data
USMBP <- read.csv(paste0(loc, "fluxnet-ch4-data/ameriflux-validation/csv/AMF_US-MBP_BASE_HH_1-5.csv"), skip = 2) %>%
  mutate(ID = "US-MBP",
         FCH4 = ifelse(FCH4 == -9999, NA, FCH4),
         Year = as.integer(substr(TIMESTAMP_END, 1, 4)),
         Month = as.integer(substr(TIMESTAMP_END, 5, 6)),
...
site.pred %>% 
  ...
  scale_x_continuous(breaks = c(1:12), limits = c(1,12))
ggsave(paste0(loc, "/predictions/gridded/nmol/figures/US-MBP.png"),
       width = 14, height = 12, units = c("cm"), dpi = 300)

# get stats
joined_mbp <- site.pred %>% 
  filter(ID == "US-MBP") %>% 
  left_join(USMBP)  %>% 
  filter(!is.na(FCH4))

cor(joined_mbp$mean_FCH4P, joined_mbp$FCH4)^2
sum(abs(joined_mbp$mean_FCH4P - joined_mbp$FCH4))/length(joined_mbp$mean_FCH4P)
sum(abs(joined_mbp$mean_FCH4P - joined_mbp$FCH4))/length(joined_mbp$mean_FCH4P)/sd(joined_mbp$FCH4) # nmae
sum(joined_mbp$mean_FCH4P - joined_mbp$FCH4)/length(joined_mbp$mean_FCH4P)
length(joined_mbp$mean_FCH4P)
```

 Tropical Africa flux comparisons (Jacob Shaw)

```{r}
# read in mg CH4 m-2 d-1 flux map - weighted by wetland area
mgch4 <- stack(paste0(loc, "upscaling-output-raw/grids/upch4_v04_m1_mgCH4m2day_Aw.nc"))
bangweulu <- c(30.083333, -11.6)
kafue <- c(27.283333, -15.716667)
lukanga <- c(27.783333, -14.483333)
coords <- rbind(bangweulu, kafue, lukanga)

plot(mgch4[[1]])
points(coords)

mgch4_coords <- extract(mgch4, coords)

# wrangle
mgch4_coords <- mgch4_coords %>% as_tibble() %>% 
  mutate(wetland = c("bangawulu", "kafue", "lukanga")) %>% 
  dplyr::select(wetland, everything()) %>% 
  gather(key = "timestep", value = "mgch4day", 2:217) %>% 
  arrange(wetland) %>% 
  mutate(year = rep(rep(2001:2018, each = 12), 3),
         month = rep(rep(1:12, 18), 3)) %>% 
  dplyr::select(year, month, wetland, mgch4day) %>% 
  mutate(mgch4hr_weighted = mgch4day / 24) %>% 
  group_by(wetland, month) %>% 
  summarize(mgch4hr_weighted = mean(mgch4hr_weighted)) 

write.csv(mgch4_coords, paste0(loc, "predictions/gridded/jacob_shaw_zambia/mgch4hr_weighted.csv"), row.names = F)
  

# read in nmol m-2 s-1 flux map - NOT weighted by wetland area
nmolch4 <- stack(paste0(loc, "upscaling-output-raw/grids/upch4_v04_m1_nmolm2sec.nc"))

nmolch4_coords <- extract(nmolch4, coords)

# wrangle
nmolch4_coords <- nmolch4_coords %>% as_tibble() %>% 
  mutate(wetland = c("bangawulu", "kafue", "lukanga")) %>% 
  dplyr::select(wetland, everything()) %>% 
  gather(key = "timestep", value = "nmolch4", 2:217) %>% 
  arrange(wetland) %>% 
  mutate(year = rep(rep(2001:2018, each = 12), 3),
         month = rep(rep(1:12, 18), 3)) %>% 
  dplyr::select(year, month, wetland, nmolch4) %>% 
  mutate(mgch4hr_unweighted = nmolch4 * 3600 * 10^-6 * 16.04) %>% 
  group_by(wetland, month) %>% 
  summarize(mgch4hr_unweighted = mean(mgch4hr_unweighted))

write.csv(nmolch4_coords, paste0(loc, "predictions/gridded/jacob_shaw_zambia/mgch4hr_unweighted.csv"), row.names = F)


```


 Calculate Global Grid Tg Totals (METHOD 1: Grid output summed means and summed sd) 

 I chose the output of this method for the global sums! For the other two methods, we do not have the individual bootstrap sums for each region, only globally
 I then adapted this method to output sums and sds for each region (the regionalization code is commented out)

```{r echo = F, warning = F, message = F}

# CHOOSE WAD2M OR GIEMS OUTPUT
tgch4 <- nc_open(paste0(loc, "upscaling-output-raw/grids/upch4_v04_m1_TgCH4month_Aw.nc"))
tgch4_mean <- ncvar_get(tgch4, "mean_ch4")
tgch4_sd <- ncvar_get(tgch4, "sd_ch4")
...
tgch4_mean[tgch4_mean == fillvalue$value] <- NA
tgch4_sd[tgch4_sd == fillvalue$value] <- NA

r_mean <- list()
r_sd <- list()
for(i in 1:length(t)) { 
  r_mean[[i]] <- raster(t(tgch4_mean[, , i]), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat), crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0"))
  r_sd[[i]] <- raster(t(tgch4_sd[, , i]), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat), crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0"))
}

r_mean_stack <- stack(r_mean)
r_sd_stack <- stack(r_sd)


# regions <- c("global", "arctic_boreal", "hbl", "wsl", "northern", "temperate", "tropics", "ppr", "amazon", "sudd", "congo", "indonesia")
regions <- c("australia", "sahel", "csamerica")

# e1 <- extent(-180, 180, -90, 90)
# e2 <- extent(-180, 180, 60, 90)
# e3 <- extent(-96, -75, 50, 60)
# e4 <- extent(60, 94.5, 52, 74) 
# e5 <- extent(-180, 180, 45, 90)
# e6 <- extent(-180, 180, 30, 60)
# e7 <- extent(-180, 180, -90, 30)
# e8 <- extent(-114.5363, -92.52392, 41.71808, 54.66925)
# e9 <- extent(-80, -50, -20, 5)
# e10 <- extent(28, 34.5, 5, 10)
# e11 <- extent(15, 23, -5, 3)
# e12 <- extent(95, 120, -4, 5)
e13 <- extent(110, 150, -40, -15)
e14 <- extent(-15, 40, 10, 20)
e15 <- extent(-65, -55, -40, -15)

# extents <- list(e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12)
extents <- list(e13, e14, e15)

for (i in 1:length(regions)){

 # Regional extents
  e <- extents[[i]]
  regional_r_mean <- crop(r_mean_stack, e)
  regional_r_sd <- crop(r_sd_stack, e)
...
upscaling_sums <- cbind(sums_mean, sums_sd) %>% 
  as_tibble() %>% 
  mutate(row = 1:n(),
         Year = rep(2001:(2001+years-1), each = 12),
         Month = rep(1:12, years)) %>% 
...
write.csv(upscaling_sums, paste0(loc, "predictions/gridded/tg/sums_method1/", regions[i], "/upscaling_sums.csv"), row.names = F)
write.csv(annual_sums, paste0(loc, "predictions/gridded/tg/sums_method1/", regions[i], "/annual_sums.csv"), row.names = F)
write.csv(monthly_cycle, paste0(loc, "predictions/gridded/tg/sums_method1/", regions[i], "/monthly_cycle.csv"), row.names = F)

}
```
 Get regional sums for GCP TD and BU
```{r}
# get file names
files <- list.files("/Volumes/Samsung_T5/Stanford/upch4_local/grids/ch4-models/gcp-ch4_ncdfs/bu_td_mean_sd/")

r_mean <- stack(paste0("/Volumes/Samsung_T5/Stanford/upch4_local/grids/ch4-models/gcp-ch4_ncdfs/bu_td_mean_sd/", files[1]))
r_sd <- stack(paste0("/Volumes/Samsung_T5/Stanford/upch4_local/grids/ch4-models/gcp-ch4_ncdfs/bu_td_mean_sd/", files[2]))


regions <- c("global", "arctic_boreal", "hbl", "wsl", "northern", "temperate", "tropics", "ppr", "amazon", "sudd", "congo", "indonesia", "australia", "sahel", "csamerica")
...
extents <- list(e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14, e15)

for (i in 1:length(regions)){

 # Regional extents
  e <- extents[[i]]
  regional_r_mean <- crop(r_mean, e)
  regional_r_sd <- crop(r_sd, e)
...
upscaling_sums <- cbind(sums_mean, sums_sd) %>% 
  as_tibble() %>% 
  mutate(row = 1:n()) %>% 
  dplyr::select(row, sums_mean, sums_sd)

write.csv(upscaling_sums, paste0(loc, "predictions/gridded/tg/sums_method1/", regions[i], "/gcp_bu_sums.csv"), row.names = F)
}
...
```
 ...
 Calculate Global Grid Tg Totals 
 (METHOD 2: Mean bootstrap sum and variance) ## PROBLEM: We cannot use this method for regional totals because we do not have the individual bootstrap sums for each region

```{r}
# read in csv bootstrap member sum from upscaling output (b = bootstrap, m = forcing data member, t = time)
# there is only one forcing member dataset 
bootstrap_sums <- read.csv(paste0(loc, "upscaling-output-raw/sums/boot_sum_TgCH4month_m1.csv")) 

bootstrap_sums %>% 
  group_by(t) %>% 
  summarize(mean_sum_TgCH4month = mean(sum_TgCH4month),
            sd_sum_TgCH4month = sd(sum_TgCH4month)) %>% 
  mutate(Year = rep(2001:2018, each = 12),
         Month = rep(1:12, 18)) %>% 
  group_by(Year) %>% 
  summarize(mean_sum_TgCH4year = sum(mean_sum_TgCH4month),
            sd_sum_TgCH4year = sum(sd_sum_TgCH4month)) %>% 
  write.csv(paste0(loc, "predictions/gridded/tg/sums_method2/upscaling_sums.csv"), row.names = F)
```

 (METHOD 3: Individual bootstrap cumsums and variance) ## PROBLEM: We cannot use this method for regional totals because we do not have the individual bootstrap sums for each region
...
bootstrap_sums %>% 
  arrange(b) %>% 
  mutate(Year = rep(rep(2001:2018, each = 12), 500),
         Month = rep(rep(1:12, 18), 500)) %>% 
  group_by(b, Year) %>% 
  summarize(cum_sum_TgCH4year = cumsum(sum_TgCH4month)) %>% 
  filter(cum_sum_TgCH4year == max(cum_sum_TgCH4year)) %>% 
  group_by(Year) %>% 
  summarize(mean_cum_sum_TgCH4year = mean(cum_sum_TgCH4year),
            sd_cum_sum_TgCH4year = sd(cum_sum_TgCH4year)) %>% 
    write.csv(paste0(loc, "predictions/gridded/tg/sums_method3/upscaling_sums.csv"), row.names = F)


# compute average across all years for each bootstrap model (for Kuang-yu manuscript)
bootstrap_sums %>% 
  arrange(b) %>% 
  mutate(Year = rep(rep(2001:2018, each = 12), 500),
         Month = rep(rep(1:12, 18), 500)) %>% 
  ...
  group_by(b) %>% 
  summarize(mean_cum_sum_TgCH4year = mean(cum_sum_TgCH4year)) %>% 
    write.csv(paste0(loc, "predictions/gridded/tg/sums_method3/upscaling_sums_by_bootstrap.csv"), row.names = F)


```

 Calculate the time series average from the output of METHOD 1

```{r}
sums <- read.csv(paste0(loc, "predictions/gridded/tg/sums_method1/tropics/annual_sums.csv"))

sums %>% 
  summarize(timeseries_sum_mean_tg = mean(sum_mean_tg),
            timeseries_sum_ci_tg = mean(sum_sd_tg))

```

 Plot bootstrap sum histograms
...
bootstrap_sums %>% 
  ...
  summarize(cum_sum_TgCH4year = cumsum(sum_TgCH4month)) %>% 
  filter(cum_sum_TgCH4year == max(cum_sum_TgCH4year)) %>% 
  ggplot(aes(cum_sum_TgCH4year)) +
  geom_histogram() +
  labs(x = expression("Upscaling Bootstrap Sums (Tg CH"[4]*" y"^{-1}*")")) +
  my_theme
ggsave(paste0(loc, "/predictions/gridded/tg/figures/annual_sums_histogram.png"),
       width = 12, height = 10, units = c("cm"), dpi = 300)
...
bootstrap_sums %>% 
  ...
  labs(x = expression("Upscaling Bootstrap Sums (Tg CH"[4]*" y"^{-1}*")")) +
  my_theme
ggsave(paste0(loc, "/predictions/gridded/tg/figures/yearly_annual_sums_histogram.png"),
       width = 20, height = 18, units = c("cm"), dpi = 300)
...
bootstrap_sums %>% 
  ...
  mutate(Year = rep(rep(2001:2018, each = 12), 500),
         Month = rep(rep(1:12, 18), 500)) %>% 
  filter(Month < 7) %>% 
  ggplot(aes(sum_TgCH4month)) +
  geom_histogram() +
  facet_wrap(~Month, ncol = 1) +
  my_theme
ggsave(paste0(loc, "/predictions/gridded/tg/figures/monthly_sums_histogram_1.png"),
       width = 12, height = 24, units = c("cm"), dpi = 300)
...
bootstrap_sums %>% 
  ...
  facet_wrap(~Month, ncol = 1) +
  my_theme
ggsave(paste0(loc, "/predictions/gridded/tg/figures/monthly_sums_histogram_2.png"),
       width = 12, height = 24, units = c("cm"), dpi = 300)
```

#### 11. (cont.) Timeseries plots of different Tg gridded products

```{r warning = F, message = F, echo = F}
# read in upscaling monthly mean and variance
upscaling <- read.csv(paste0(loc, "predictions/gridded/tg/sums_method1/global/upscaling_sums.csv")) %>% 
  dplyr::select(-row) %>% 
  mutate(wetlands = "WAD2M")

# read in GIEMS-2 upscaling monthly mean and variance
upscaling_giems <- read.csv(paste0(loc, "predictions/gridded/tg/sums_method1/global/upscaling_sums_giems2.csv")) %>% 
  dplyr::select(-row) %>% 
  mutate(wetlands = "GIEMS-2")

# combine
upscaling <- bind_rows(upscaling, upscaling_giems)

## CarbonTracker no longer used, replaced with Saunois et al 2020 TD ensemble subset
# # read in carbon tracker (Top down model)
# files <- list.files(paste0(loc, "grids/ch4-models/carbontracker/"))
# 
# # check units
# nc_open(paste0(loc, "grids/ch4-models/carbontracker/", files[1])) ##       units: mgCH4/m2/day (`natural` variable)
# # date range is 2000-2010
# 
# ctracker <- stack(paste0(loc, "grids/ch4-models/carbontracker/", files), varname = "natural")
# ctracker_tg <- ctracker * area(ctracker) * 10^6 * 30 # area and time conversion 
# ctracker_tg_sums <- cellStats(ctracker_tg, sum) / 10^15 
# ctracker_tg_sums_vector <- unname(ctracker_tg_sums)
# ctracker_tg_sums_all <- ctracker_tg_sums_vector %>% 
#   as_tibble() %>% 
#   mutate(Year = rep(2000:2010, each = 12), 
#          Month = rep(1:12, 11)) %>% 
#   rename("ctracker" = "value")
#   

# read in gcp models
files <- list.files(paste0(loc, "grids/ch4-models/gcp-ch4_ncdfs/prescribed/"))

# # check units
# nc_open(paste0(loc, "grids/ch4-models/gcp-ch4_ncdfs/prescribed/", files[1])) ##       units: gCH4/m2/month
# date range is 2000-2017 

gcp <- list()
gcp_tg <- list()
gcp_tg_sums <- list()
for(i in 1:length(files)){
  gcp[[i]] <- stack(paste0(loc, "grids/ch4-models/gcp-ch4_ncdfs/prescribed/", files[i]))
  gcp_tg[[i]] <- gcp[[i]] * area(gcp[[i]]) * 10^6
  gcp_tg_sums[[i]] <-  cellStats(gcp_tg[[i]], sum) / 10^12
}

gcp_tg_sums_vector <- list()
for(i in 1:length(files)){
  gcp_tg_sums_vector[[i]] <- unname(gcp_tg_sums[[i]])
}

gcp_tg_sums_all <- bind_cols(gcp_tg_sums_vector) 
names(gcp_tg_sums_all) <- substr(files, 1, 10)

gcp_tg_sums_all <- gcp_tg_sums_all %>% 
  mutate(Year = rep(2000:2017, each = 12), 
         Month = rep(1:12, 18)) %>% 
  rowwise() %>% 
  mutate(gcp_mean = mean(c_across(1:13)),
         gcp_sd = sd(c_across(1:13)))

# read in top-down gridded sums data
gcp_td <- read_csv(paste0(loc, "upscaling-output-raw/data_for_figures_Jan2022/td_ch4_sum_2010_2017_indiv.csv"), col_select = c(2,3,4)) %>%
  mutate(date = as_date(date),
         Year = year(date),
         Month = month(date)) %>% 
  pivot_wider(names_from = model,
              values_from = flux_TgMonth) %>% 
  dplyr::select(-date)  %>% 
  rowwise() %>% 
  mutate(gcp_td_mean = mean(c_across(2:19)),
         gcp_td_sd = sd(c_across(2:19)))

upscaling %>% 
  left_join(gcp_tg_sums_all) %>% 
  left_join(gcp_td) %>% 
  left_join(ctracker_tg_sums_all) %>% 
  mutate(Dec_year = Year + Month/12) %>%
  write.csv(paste0(loc, "predictions/gridded/tg/model_timeseries/gridded_sums_all.csv"), 
            row.names = F)
```

# Creating the timeseries figure

```{r}
upscaling <- read.csv(paste0(loc, "predictions/gridded/tg/model_timeseries/gridded_sums_all.csv"))

upscaling %>% 
  mutate(wetlands = ifelse(wetlands == "WAD2M", "UpCH4-WAD2M", "UpCH4-GIEMS-2")) %>% 
  mutate(Dec_year = Year + Month/12) %>%
  ggplot(aes(Dec_year, sums_mean, linetype = wetlands)) +
  
  geom_ribbon(aes(ymin = gcp_mean - gcp_sd, ymax = gcp_mean + gcp_sd), fill = 'orange', alpha = 0.2) + 
  geom_ribbon(aes(ymin = gcp_td_mean - gcp_td_sd, ymax = gcp_td_mean + gcp_td_sd), fill = 'dark green', alpha = 0.1) + 
  
    geom_line(aes(Dec_year, `gcp_mean`), color = 'orange', alpha = 1, size = 0.3, linetype = "dashed") +
  geom_line(aes(Dec_year, `gcp_td_mean`), color = 'dark green', alpha = 1, size = 0.3, linetype = "longdash") +

    geom_ribbon(aes(ymin = sums_mean - sums_sd, ymax = sums_mean + sums_sd, fill = wetlands), alpha = 0.1) + # add observations
    geom_line(aes(color = wetlands), size = 0.3) +
  
  scale_y_continuous(limits = c(0, 30)) +
  scale_x_continuous(breaks = c(seq(2000, 2018, 2))) +
  scale_linetype_manual(values = c("dotdash", "solid")) +
  scale_fill_manual(values = c("yellow", "purple")) +
  scale_color_manual(values = c("black", "purple")) +
  
  labs(x = "Year", y = expression("Wetland Emissions (Tg "*CH[4]*~month^{-1}*")")) +
  my_theme
ggsave(paste(loc, "/predictions/gridded/tg/figures/model_timeseries.png", sep = ""),
         width = 25, height = 16, units = c("cm"), dpi = 300)
```



 Compare annual totals for GCP and Carbon Tracker, and NSE values

```{r}
grid_sums <- read.csv(paste0(loc, "predictions/gridded/tg/model_timeseries/gridded_sums_all.csv"))

# BU mean sum
grid_sums %>% 
  group_by(Year, wetlands) %>% 
...
grid_sums %>% 
  ...
  filter(complete.cases(.)) %>% 
  group_by(wetlands, model) %>% 
  summarize(R2 = cor(emissions, sums_mean)^2,
            MADiff = sum(abs(sums_mean - emissions))/n(),
            Diff = mean(emissions - sums_mean)) %>% 
  arrange(MADiff) %>% 
  write.csv(paste0(loc, "predictions/gridded/tg/model_timeseries/gcp_metrics.csv"), row.names = F)

## sanity check some low R2 for ELM and LPJ.MPI
...
grid_sums %>% 
  ...
  filter(wetlands == "WAD2M") %>% 
  # filter(model %in% c("LPJ.MPI_ec", "ELM_ech4_c")) %>% 
  ggplot(aes(Dec_year, emissions)) +
  geom_line() +
  geom_line(aes(Dec_year, sums_mean), color = "red") +
  facet_wrap(~model, ncol = 2) +
  labs(x = "Year", y = expression("Wetland Emissions (Tg "*CH[4]*~month^{-1}*")")) +
  my_theme
ggsave(paste(loc, "/predictions/gridded/tg/figures/individual_bu_model_timeseries_comparisons.png", sep = ""),
...
grid_sums %>% 
  ...
  mutate(model = str_remove(model, "_post_GCP2019_MASK11.nc"),
         model = str_remove(model, "_post_GCP2019_MASK11_v2.nc")) %>% 
  filter(wetlands == "WAD2M",
         Year %in% c(2010:2017)) %>% 
  # filter(model %in% c("LPJ.MPI_ec", "ELM_ech4_c")) %>% 
  ggplot(aes(Dec_year, emissions)) +
  ...
  my_theme
ggsave(paste(loc, "/predictions/gridded/tg/figures/individual_td_model_timeseries_comparisons.png", sep = ""),
         width = 20, height = 30, units = c("cm"), dpi = 300)
  
```


#### 11. (cont.) Plots of Global and Regional Totals (Tg)

```{r}
# load global and regional comparison dataset
fig6_sums <- read.csv(paste0(loc, "predictions/gridded/tg/Figure6 Data/Tidy Tg Comparions for Fig 6.csv"))

# get colors
qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
col_vector = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))
col_vector <-  c("#33A02C","#377EB8","black","#E78AC3")

# make global figure
fig6_sums %>% 
  arrange(order) %>% 
  filter(domain == "Global") %>% 
  mutate(ref = factor(ref, levels = ref)) %>%  
  ggplot(aes(ref, annual, shape = class, color = class)) +
    geom_hline(
      yintercept = fig6_sums[which(fig6_sums$ref == "McNicol et al. WAD2M"
                                   & fig6_sums$domain == "global"), 8],
      linetype = 2, color = "grey") +
  geom_point(size = 4) +
  geom_errorbar(aes(ref, ymin = lower, ymax = upper, width = 0)) +
  # geom_polygon(aes(x = c(1,10), y = c(2.1, 3.4))) +
  labs(x = "Reference", y = expression(atop("Global CH"[4]*" Emissions",
                                  "(Tg yr"^{-1}*")"))) +
  scale_color_manual(values = col_vector) +
  # scale_shape_manual(values = c(16, 15, 3)) +
  coord_flip() +
  my_theme
ggsave(paste0(loc, "predictions/gridded/tg/figures/global_comparison.png"),
...
#### 11. (cont.) Gridded variance vs. dissimilarity (not used!)

 Prediction variance appears to be much more related to FCH4 variance for tower constituencies

```{r}
# read in upscaling sd
ch4 <- nc_open(paste0(loc, "upscaling-output-raw/grids/upch4_v04_m1_nmolm2sec.nc"))
ch4_sd <- ncvar_get(tgch4, "sd_ch4")

lon <- ncvar_get(ch4, "longitude")
lat <- ncvar_get(ch4, "latitude", verbose = F)
t <- ncvar_get(ch4, "Time")

fillvalue <- ncatt_get(ch4, "mean_ch4", "_FillValue")

nc_close(ch4) 

ch4_sd[ch4_sd == fillvalue$value] <- NA

r_sd <- list()
for(i in 1:216) {
  r_sd[[i]] <- raster(t(ch4_sd[, , i]), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat), crs=CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0"))
}
...
# read in global dissimilarity
di <- read.csv(paste0(loc, "dissimilarity/output/stats/all_monthly_global_DI7.csv"))
x <- raster(xmn = -179.875, xmx = 179.875, ymn = -55.875, ymx = 84.875, res = c(0.2498264, 0.2495567), crs="+proj=longlat +datum=WGS84")
di_global <- rasterize(di[, c('x', 'y')], x, di[, 'DI'])
extent(di_global) <- extent(r_sd_stack[[7]])
plot(di_global)

# create stack
sd_di_global <- stack(r_sd_stack[[7]], di_global)

random_x <- sample(seq(-180, 180, 0.25), size = 5000, replace = T)
random_y <- sample(seq(-55, 85, 0.25), size = 5000, replace = T)
coords <- cbind(random_x, random_y)

sd_di_extract <- extract(sd_di_global, coords) %>%
  as_tibble() %>% 
  filter(!is.na(layer.7) & !is.na(layer)) %>% 
  rename("StDev" = layer.7, "DI" = layer)
